  ...
  for (var i = 0; i < row; i++) {
      ...
      if(document.getElementById('times').checked == true){
        ...
        //   console.log(res);
          ...
          ctx.clearRect(0, 0, canvas.width, canvas.height);
          ctx.fillStyle = "black";
          ctx.fillText(mtextA,textPosX,textPosY);
          ctx.fillText(mtextB,textPosX*row+410/row,textPosY);
          ctx.fillText("x",380,textPosY*row);
          displayMatrices(arrCellsA,0,mPosX);
          displayMatrices(arrCellsB,mPosY,0);
          ctx.translate(-mPosY,-mPosX);
          ctx.fillStyle = "blue";
          ctx.fillText(text2,textPosX,500);
          displayMatrices(matrix,0,mPosY+200);
          ctx.translate(0,-610);
        ...
        ctx.fillText(mtextA,textPosX,textPosY);
        displayMatrices(arrCellsA,0,mPosX);
        displayMatrices(matrix,mPosY,0);
        ctx.translate(-mPosY,-mPosX);
        ctx.fillStyle = "blue";
        ctx.fillText("Transpose A",textPosX*row+450/row,textPosY);
        console.log(mPosY);
      }
  for(i = 1; i < row+1;i++){
...
// This function takes two matrices (a and b) as input and returns a new matrix that is the sum of a and b.
function addingMatrices(a,b){
  // Create an empty array to store the result
  var result = [];
  // Loop through each row of the matrices
  for (var i = 0; i < a.length; i++) {
    // Create an empty array to store the row of the result matrix
    result[i] = [];
    // Loop through each column of the matrices
    for (var j = 0; j < a[0].length; j++) {
      // Add the corresponding elements of the two input matrices and store the result in the corresponding element of the result matrix
...
console.log(matrix[0].length * cellSize);
